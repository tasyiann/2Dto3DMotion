...
namespace RootMotion.Dynamics {
	...
	public abstract class RagdollCreator: MonoBehaviour {
		...
		public static void ClearAll(Transform root) {
			...
			Transform r = animator != null && animator.isHuman? animator.GetBoneTransform(HumanBodyBones.Hips): root;
		...
		protected static void ClearTransform(Transform transform) {
			...
			if (transform.GetComponent<Collider>() != null) GameObject.DestroyImmediate(transform.GetComponent<Collider>());
			if (transform.GetComponent<Joint>() != null) GameObject.DestroyImmediate(transform.GetComponent<Joint>());
			if (transform.GetComponent<Rigidbody>() != null) GameObject.DestroyImmediate(transform.GetComponent<Rigidbody>());
		}
		...
		protected static void CreateCollider(Transform t, Vector3 startPoint, Vector3 endPoint, ColliderType colliderType, float lengthOverlap, float width) {
			...
			case ColliderType.Capsule:
				...
				capsule.height = height / scaleF;
				capsule.radius = (width * 0.75f) / scaleF;
