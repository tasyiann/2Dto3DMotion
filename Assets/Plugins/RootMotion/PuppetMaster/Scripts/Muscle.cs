...
namespace RootMotion.Dynamics {
	...
	public class Muscle {
		...
		public void UpdateColliders() {
			_colliders = new Collider[joint.GetComponent<Collider>() != null? 1: 0];
			if (_colliders.Length == 1) _colliders[0] = joint.GetComponent<Collider>();
		...
		private void AddCompoundColliders(Transform t, ref Collider[] colliders) {
			...
			if (t.GetComponent<Collider>() != null) {
				if (t.GetComponent<Collider>() is MeshCollider) {
				} else {
					Array.Resize(ref colliders, colliders.Length + 1);
					colliders[colliders.Length - 1] = t.GetComponent<Collider>();
				}
			}
		...
		public void Map(float mappingWeightMaster) {
			...
			if (w >= 1f) {
				target.rotation = rigidbody.rotation * targetRotationRelative;
				
				if (props.mapPosition) {
					if (connectedBodyTransform != null) {
						// Mapping in local space of the parent
						Vector3 relativePosition = connectedBodyTransform.InverseTransformPoint(rigidbody.position);
			...
			target.rotation = Quaternion.Lerp(target.rotation, rigidbody.rotation * targetRotationRelative, w);
			...
			if (props.mapPosition) {
				if (connectedBodyTransform != null) {
					...
					target.position = Vector3.Lerp(target.position, rigidbody.position, w);
				}
		...
		private void MuscleRotation(float muscleWeightMaster, float muscleSpring, float muscleDamper) {
			...
			slerpDrive.mode = w > 0f? JointDriveMode.PositionAndVelocity: JointDriveMode.None;
