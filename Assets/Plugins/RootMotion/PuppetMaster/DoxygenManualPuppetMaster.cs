...
<b>Behaviours</b>
	- <b>behaviourRoot</b> - the parent Transform for all Puppet Behaviours.
	- <b>walkableLayers</b> - collisions with those layers will be classified as CollisionType.Ground by the MuscleCollisionBroadcasters and can then be processed as such by the Puppet Behaviours. For example SubBehaviourCOM will use the ground collisions to find the center of pressure for the character.
	- <b>collisionLayers</b> - collisions with those layers will be classified as CollisionType.Collision by the MuscleCollisionBroadcasters and can then be processed as such by the Puppet Behaviours. BehaviourPuppet for example will unpin the muscles that collide with those layers.
	- <b>collisionThreshold</b> - the collision impulse sqrMagnitude threshold under which MuscleCollisionBroadcasters on the muscles will not broadcast collision events to Puppet Behaviours.
	- <b>targetRoot</b> - the root Transform of the animated target character.

<b>Muscle variables:</b>
...
Props used with the PropRoot component need to extend the abstract Prop.cs class. For an example, please see the PropTemplate.cs script.
	...
	- <b>additionalPinProps</b> - the muscle properties that will be applied to the additional pin Muscle on pickup.
...
To use PuppetMaster with Final IK, please download and import this package <a href="http://root-motion.com/addons/PuppetMasterFinalIKBridge.unitypackage">this package</a> to a project with both assets already inside.
...
or disable them and update their solvers manually in OnPuppetMasterRead:
...
OnPuppetMasterRead is called automatically by the PuppetMaster on all gameobjects in the Target hierarchy.
...
Final IK components can be updated whenever necessary and in this case we need to do it in OnPuppetMasterWrite:
...
OnPuppetMasterWrite is called automatically by the PuppetMaster on all gameobjects in the Target hierarchy.
*/
...
<b>Losing Balance and Getting Up</b>
...
<b>Activation</b>
	- <b>canActivate</b> - if true, will set the PuppetMaster.mode to Active when any of the muscles collides with something (static colliders are ignored if 'Activate On Static Collisions' is false) or is hit.
	- <b>activateOnStaticCollisions</b> - if false, static colliders will not activate the puppet when they collide with the muscles. Note that the static colliders need to have a kinematic Rigidbody attached for this to work.
	- <b>activateOnImpulse</b> - minimum collision impulse for activating the puppet.
	- <b>deactivateAutomatically</b> - if true, the PuppetMaster.mode will be set to Kinematic when all muscles are pinned and the puppet is in normal Puppet state. This will increase performance and enable you to have full animation accuracy when PuppetMaster is not needed. Note that collision events are only broadcasted if one of the Rigidbodies is not kinematic, so if you need 2 characters to wake up each other, one has to be active. 'Can Activate' should be true if you wish to reactivate automatically when the puppet collides with somethinf or is hit.
	- <b>mapOnlyOnContact</b> - if true, mapping is blended in only if the puppet becomes in contact with something and blended out again to maintain 100% animation accuracy for the rest of the time.
	- <b>mappingBlendSpeed</b> - the speed of blending in mapping in case of contact.
